...
function expandObjectOrArray(obj, pancake)
{
    var toExpand = [];
    toExpand.push(obj);
    ...
    for(var i = 0; i < toExpand.length; i++) {
        ...
        var currentExpansion = toExpand[i];

        _.forEach(currentExpansion, function (mValue, mKey) {

            //Check if it is a reference/link to another object type in the pancake
            if(_.isArray(mValue)) {

                var referencedObj = "%LOST_REF%";
                if(mValue.length == 1 && mValue[0] < pancake.length)
                {
                    referencedObj = pancake[mValue[0]];
                    toExpand.push(referencedObj);
                }
                currentExpansion[mKey] = referencedObj;
            }
...
exports.unflatten = function(pancake)
{
    ...
    if(pancake.length > 0)
    {
        ...
        if(_.isPlainObject(expanded) || _.isArray(expanded)) {
            expandObjectOrArray(expanded, pancake);
        }
