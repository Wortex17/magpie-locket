...
function reconstructObjectOrArray(pancake, indexOfStartObj, onConstruct, onRelink)
{
    ...
    {
        onConstruct = _.identity;
    }

    if(!_.isFunction(onRelink))
    {
        onRelink = function(object, links){
    ...
    for(var i = 0; i < indicesToConstruct.length; i++) {
        ...
        if(Buffer.isBuffer(flatObject))
        {
            ...
            _.forEach(flatObject, function (mValue, mKey) {
                ...
                if(_.isArray(mValue)) {
                    ...
                    {
                        indicesToConstruct.push(mValue[0]);
                        links[mKey] = [mValue[0]];
                    } else {
                        links[mKey] = -1;
                    }
        ...
        constructedObjects[pancakeIndexToConstruct] = onConstruct(reconstructionObject);
    }
    ...
    _.forEach(objectsLinks, function (links, pancakeIndex) {
        ...
        onRelink(constructedObjects[pancakeIndex], resolvedLinks);
    });
