...
function rollObjectIntoPancake(obj, pancake)
{
    _.forIn(obj, function(mValue, mKey) {
        if(_.isPlainObject(mValue) || _.isArray(mValue) || isLeafObject(mValue)) {
...
/**
 * Returns if this object is a leaf that is not supposed to carry further references
 * @param obj
...
function reconstructObjectOrArray(pancake, indexOfStartObj, onConstruct, onRelink)
{
    ...
    for(var i = 0; i < indicesToConstruct.length; i++) {
        ...
        var links = {};
        if(isLeafObject(flatObject))
        {
            //Take buffers, dates, etc. as-is
...
exports.sanitizeFromBSON = function(pancake)
{
    _.forEach(pancake, function(pancakeValue, pancakeKey, pancake){
        if(_.isObject(pancakeValue) && !_.isNull(pancakeValue) && pancakeValue._bsontype == 'Binary')
        {
            pancake[pancakeKey] = pancakeValue.buffer;
        }
    });
};