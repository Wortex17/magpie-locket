...
function addField(locket, fieldName)
{
    if(!hasField(locket, fieldName))
    {
        locket.fields[fieldName] = createFieldObject();
    }
...
/***
 * Tests if the locked locket can be opened with this keypair
 * @param {MagpieLocketLocked} lockedLocket
 * @param {NodeRSA} keypair
 * @return {Boolean}
 */
function testLock(lockedLocket, keypair)
{
    var shiftingLock = lockedLocket.c;
    try {
        var cipherKey = unlockShiftingLock(shiftingLock, keypair);
        return Buffer.isBuffer(cipherKey);
    } catch(e) {
        return false;
    }
...
/**
 * Uses the rsa keypair to generate and encrypt a new cipherKey.
 * @param {NodeRSA} keypair
 * @return {Array} - [0] is the plain cipherKey (should be thrown away after using), [1] is the encrypted cipherKey
 */
...
/**
 * Returns the decrypted cipherKey, if it was encrypted with the given keypair
 * @param shiftingLock - the encrypted cipherKey, as binary string
 * @param {NodeRSA} keypair
 * @return {Buffer|Object|string|Buffer|*}
 */
